...
(function () {
  ...
  AbstractChosen = (function () {
      ...
      for (i = 0, len = ref.length; i < len; i++) {
        ...
        if (this.include_option_in_results(option)) {
          ...
          if (!(option.group && !this.group_search)) {
            if (korean.test(regex)) {
              // 입력 언어가 한국어를 포함하고 있을 때
              search_match = this.search_korean_match(text, query);
              option.search_match = search_match.length !== 0;
              if (option.search_match && !option.group) {
                results += 1;
              }
              if (option.search_match) {
                if (query.length) {
                  startpos = search_match[0][0];
                  match_len = search_match[0][1] - search_match[0][0] + 1;
                  prefix = text.slice(0, startpos);
                  fix = text.slice(startpos, startpos + match_len);
                  suffix = text.slice(startpos + match_len);
                  option.highlighted_html =
    ...
    AbstractChosen.prototype.search_korean_match = function (
      search_string,
      query
    ) {
      // 한국어 자음 검색시 결과값이 뜰 수 있도록 하기 위한 함수
      function initialConsonant(word, consonant) {
        ...
        return Hangul.rangeSearch(conso, consonant);
      }

      const match = Hangul.rangeSearch(search_string, query);
      if (match.length !== 0) {
        return match;
      } else {
        return initialConsonant(search_string, query);
      }
    };
